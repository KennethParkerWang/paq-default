...
Encoder::Encoder(Predictor* predictorBlock, Predictor* predictorMain,
                 bool doEncoding, Mode m, File *f, Shared* sharedState) :
  ari(f),
  mode(m),
  archive(f),
  alt(nullptr),
...
{
  if (shared == nullptr)
    quit("Encoder requires shared state.");
  if( mode == DECOMPRESS ) {
    uint64_t start = size();
    archive->setEnd();
    uint64_t end = size();
...
Shared* Encoder::getShared() const { return shared; }
