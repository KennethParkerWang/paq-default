...
int ContextModel::p() {
  ...
  if( bpos == 0 ) {
    ...
    if (blpos == 0) {
      ...
      else if (blockType == BlockType::RECORD) {
        const structured::RecordInfo recordInfo =
            structured::unpackRecordInfo(static_cast<uint32_t>(blockInfo));
        RecordModel& recordModel = models->recordModel();
        // A fixed original stride is meaningful only while bytes stay in AoS
        // order. Transposed payloads retain Generic's automatic detection.
        recordModel.setParam(
            recordInfo.transform == structured::RecordTransform::MODEL_ONLY
                ? recordInfo.stride
                : 0);
      }
