...
#include "../Utils.hpp"
#include "Filter.hpp"
#include "BlockPlan.hpp"
#include "DefaultStructureDetector.hpp"
...
static uint64_t decodeFunc(BlockType type, Encoder &en, File *tmp, uint64_t len, int info, File *out, FMode mode, uint64_t &diffFound, const TransformOptions* const transformOptions) {
  if (structured::isStructuredType(type)) {
    structured::validateStructuredInfo(type, static_cast<uint32_t>(info));
    auto f = StructuredDataFilter(type, info);
    f.setEncoder(en);
    return f.decode(nullptr, out, mode, len, diffFound);
  }
...
static uint64_t encodeFunc(BlockType type, File *in, File *tmp, uint64_t len, int info, int &hdrsize, const TransformOptions* const transformOptions) {
  ...
  } else if( type == BlockType::IMAGE24 ) {
    auto f= BmpFilter();
    f.setSkipRgb(transformOptions->skipRgb);
    f.encode(in, tmp, len, info, hdrsize);
...
transformEncodeBlock(BlockType type, File *in, uint64_t len, Encoder &en, int info, String &blstr, float p1, float p2, uint64_t begin, const TransformOptions* const transformOptions) {
  ...
  if (structured::isStructuredType(type) && hasTransform(type, info)) {
    structured::validateStructuredInfo(type, static_cast<uint32_t>(info));
    FileTmp tmp;
    int headerSize = 0;
    const uint64_t transformError =
        encodeFunc(type, in, &tmp, len, info, headerSize, transformOptions);
    const uint64_t transformedSize = tmp.curPos();
    if (transformError != 0 || headerSize != 0 || transformedSize != len)
      quit("Internal structured transform length invariant failed.");
    tmp.setpos(0);
    directEncodeBlock(type, &tmp, transformedSize, en, info);
    tmp.close();
    return;
  }
...
static void printBlock(const uint64_t begin, const uint64_t len, const BlockType type, const int blockInfo, String& blstrSub) {
  static const char* typeNames[] = { "default", "jpeg", "hdr", "1b-image", "4b-image", "8b-image", "8b-img-grayscale",
                                      ...
                                      "dBase", "base85", "tar", "tar header", "record", "numeric", "wide-text"};
  static_assert(sizeof(typeNames) / sizeof(typeNames[0]) == static_cast<size_t>(BlockType::Count),
                "Block type names must match BlockType");
  ...
  if (type == BlockType::AUDIO || type == BlockType::AUDIO_LE) {
    ...
    const structured::RecordInfo recordInfo =
        structured::unpackRecordInfo(static_cast<uint32_t>(blockInfo));
    printf(" (stride: %u, mode: %u)", static_cast<unsigned>(recordInfo.stride),
           static_cast<unsigned>(recordInfo.transform));
  }
  else if (type == BlockType::NUMERIC) {
    const structured::NumericInfo numericInfo =
        structured::unpackNumericInfo(static_cast<uint32_t>(blockInfo));
    printf(" (%u-byte %s, row: %u, mode: %u)",
           static_cast<unsigned>(numericInfo.elementBytes),
           numericInfo.bigEndian ? "be" : "le",
           static_cast<unsigned>(numericInfo.rowWidthElements),
...
static BlockPlan buildBlockPlan(File* in, uint64_t bytesToProcess,
                                const TransformOptions* const transformOptions) {
  if (in == nullptr)
    quit("Cannot plan blocks without an input file.");

  const uint64_t sourceBegin = in->curPos();
  BlockPlan plan(sourceBegin, bytesToProcess);
  uint64_t begin = sourceBegin;
  uint64_t remaining = bytesToProcess;

  while (remaining > 0) {
    const DetectionInfo detectionInfo = detect(in, remaining, transformOptions);
    in->setpos(begin);

    const uint64_t blockStart = detectionInfo.HeaderLength != 0
      ? detectionInfo.HeaderStart : detectionInfo.DataStart;
    if (blockStart < begin || blockStart > plan.sourceEnd())
      quit("Block detector returned a position outside the planned source range.");

    while (blockStart != begin) {
      const TextDetectionInfo textDetectionInfo =
          detectText(in, begin, blockStart - begin); // DEFAULT / TEXT / TEXT_EOL
      if (textDetectionInfo.DataStart != begin || textDetectionInfo.DataLength == 0 ||
          textDetectionInfo.DataLength > blockStart - begin)
        quit("Text detector returned a non-contiguous block plan span.");

      BlockType selectedType = textDetectionInfo.Type;
      int selectedInfo = 0;
      if (selectedType == BlockType::DEFAULT) {
        const SecondaryDecision secondary = detectDefaultStructure(
    ...
    if (detectionInfo.HeaderLength != 0) {
      if (detectionInfo.HeaderStart != begin || detectionInfo.HeaderLength > remaining)
        quit("Special-block detector returned an invalid header span.");
      plan.append(detectionInfo.HeaderStart, detectionInfo.HeaderLength,
                  BlockType::HDR, 0);
      begin += detectionInfo.HeaderLength;
      remaining -= detectionInfo.HeaderLength;
    }

    if (begin != detectionInfo.DataStart)
      quit("Special-block detector returned a non-contiguous data span.");
    if (detectionInfo.DataLength > remaining)
      quit("Special-block detector returned a block longer than the remaining input.");

    if (detectionInfo.DataLength != 0) {
      plan.append(detectionInfo.DataStart, detectionInfo.DataLength,
                  detectionInfo.Type, detectionInfo.DataInfo);
      begin += detectionInfo.DataLength;
      remaining -= detectionInfo.DataLength;
    }
    else if (detectionInfo.HeaderLength == 0 && remaining != 0) {
      quit("Block detector made no progress while building the block plan.");
    }
    in->setpos(begin);
  }

  plan.seal();
  in->setpos(sourceBegin);
  return plan;
}
...
static void compressRecursiveForTar(File* in, uint64_t bytesToProcess, Encoder& en, String& blstr, float p1, float p2, const TransformOptions* const transformOptions) {
  ...
  if (filePositions.size() == 0 ||
      filePositions[filePositions.size() - 1] != bytesToProcess)
    quit("TAR planning did not cover the transformed stream.");
  in->setpos(0);

  BlockPlan tarPlan(0, bytesToProcess);
  const uint64_t headerSize = filePositions[0];
  if (headerSize != 0)
    tarPlan.append(0, headerSize, BlockType::TARHDR, 0);
  for (size_t i = 1; i < filePositions.size(); ++i) {
    if (filePositions[i] <= filePositions[i - 1])
      quit("TAR planning returned unordered member boundaries.");
    tarPlan.append(filePositions[i - 1],
                   filePositions[i] - filePositions[i - 1],
                   BlockType::DEFAULT, 0);
  }
  tarPlan.seal();
  ...
  size_t planIndex = 0;
  if (headerSize != 0) {
    const PlannedBlock& header = tarPlan[planIndex++];
  ...
  for (; planIndex < tarPlan.size(); ++planIndex, ++blNum) {
    ...
    p2 = p1 + pscale * member.sourceLength;
    en.setStatusRange(p1, p2);
    String blstrSub;
    composeSubBlockStringToPrint(blstr, blstrSub, blNum);
...
static void compressfile(const Shared* const shared, const char *filename, uint64_t fileSize, Encoder &en, bool verbose) {
  ...
  if (forcedBlockType != BlockType::Count) {
    // Forced DEFAULT/TEXT bypasses detection but still passes through the same
    // coverage-checked plan/encode boundary.
    BlockPlan plan(0, fileSize);
    if (fileSize != 0)
      plan.append(0, fileSize, forcedBlockType, -1);
    plan.seal();
    encodeBlockPlan(&in, plan, en, blstr, 0.0f, 1.0f, &transformOptions);
  }
  else {
